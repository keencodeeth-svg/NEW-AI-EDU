...
function buildUsers() {
  ...
  return {
    teachers: teachers.map(normalizeSeedUser),
    students: students.map(normalizeSeedUser),
    parents: parents.map(normalizeSeedUser)
  };
}
  ...
  try {
    ...
    for (const moduleRecord of modules) {
      await client.query(
        ...
        [
          moduleRecord.id,
          moduleRecord.classId,
          moduleRecord.parentId ?? null,
          moduleRecord.title,
          moduleRecord.description ?? null,
          moduleRecord.orderIndex ?? 0,
