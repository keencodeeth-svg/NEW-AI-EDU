...
const usersSeedByEmail = new Map(usersSeed.map((user) => [user.email, user]));
...
async function seedJson() {
  ...
  const teacher = users.find((u) => u.email === PRIMARY_TEACHER.email) ?? usersSeedByEmail.get(PRIMARY_TEACHER.email) ?? PRIMARY_TEACHER;
  const teacher2 =
    users.find((u) => u.email === SCHEDULE_TEACHER_2.email) ?? usersSeedByEmail.get(SCHEDULE_TEACHER_2.email) ?? SCHEDULE_TEACHER_2;
  const teacher3 =
    users.find((u) => u.email === SCHEDULE_TEACHER_3.email) ?? usersSeedByEmail.get(SCHEDULE_TEACHER_3.email) ?? SCHEDULE_TEACHER_3;
  const student = users.find((u) => u.email === PRIMARY_STUDENT.email) ?? usersSeedByEmail.get(PRIMARY_STUDENT.email) ?? PRIMARY_STUDENT;
  const student2 = users.find((u) => u.email === SECOND_STUDENT.email) ?? usersSeedByEmail.get(SECOND_STUDENT.email) ?? SECOND_STUDENT;
  const student3 = users.find((u) => u.email === THIRD_STUDENT.email) ?? usersSeedByEmail.get(THIRD_STUDENT.email) ?? THIRD_STUDENT;
...
async function seedDb() {
  ...
  try {
    ...
    const teacherId = await ensureUser(client, usersSeedByEmail.get(PRIMARY_TEACHER.email) ?? PRIMARY_TEACHER);
    const teacher2Id = await ensureUser(client, usersSeedByEmail.get(SCHEDULE_TEACHER_2.email) ?? SCHEDULE_TEACHER_2);
    const teacher3Id = await ensureUser(client, usersSeedByEmail.get(SCHEDULE_TEACHER_3.email) ?? SCHEDULE_TEACHER_3);
    const studentId = await ensureUser(client, usersSeedByEmail.get(PRIMARY_STUDENT.email) ?? PRIMARY_STUDENT);
    const student2Id = await ensureUser(client, usersSeedByEmail.get(SECOND_STUDENT.email) ?? SECOND_STUDENT);
    const student3Id = await ensureUser(client, usersSeedByEmail.get(THIRD_STUDENT.email) ?? THIRD_STUDENT);
    await ensureUser(client, {
      ...(usersSeedByEmail.get(PRIMARY_PARENT.email) ?? PRIMARY_PARENT),
      studentId
    });
