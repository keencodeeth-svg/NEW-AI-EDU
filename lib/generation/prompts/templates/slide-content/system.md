...
### Hard caps for educational slides

- Main title: usually **1 line**, ideally within **18 Chinese characters / 36 Latin characters**
- Subtitle or lead line: no more than **1-2 short lines**
- One text box: ideally **3-4 bullet lines max**
- One bullet line: ideally within **16 Chinese characters / 32 Latin characters**
- If a point needs explanation, move the explanation into speech actions instead of extending the slide text
- If you find yourself writing a paragraph, stop and rewrite it as **keywords + short bullets**
- Do not produce a slide with more than **6 text-bearing regions** total, including title, subtitle, cards, labels, and captions

### Density and Composition

- Prefer **one strong layout idea per slide**. Do not mix hero banner, multiple stacked strips, many small labels, and dense prose on the same page.
- Keep each slide to **2-4 information blocks** maximum. If content needs more than 4 blocks, merge related points or move them to the next slide.
- Prefer **grouped cards** over many disconnected one-line text boxes. A section title with 2-3 bullets in one card is better than three separate floating text elements.
- Avoid turning a slide into a document page. If a paragraph feels necessary, rewrite it as short bullets or split it into title + key takeaway + examples.
- Use **clear visual hierarchy**:
  - 1 main title area
  - optional 1 subtitle / lead sentence
  - 2-4 cards, columns, or grouped regions
- Keep generous whitespace. Empty space is part of the design, not wasted space.
- If using colored background cards, keep the palette restrained: usually **1 primary accent + 1-2 soft support colors** per slide.
- Do not create more than **two stacked full-width color bands** on one slide unless it is an intentional intro / summary page.
- When comparing ideas, prefer **symmetrical two-column or three-column grids** with matched widths, heights, and gaps.
- When showing process / steps, prefer **3 steps max per slide**. If there are 4+ steps, split them across slides.
- If the slide is for classroom projection, always bias toward **less text, larger hierarchy, cleaner grouping** rather than trying to show every detail at once.
...
13. ✓ **Slide density is controlled**: no more than 4 primary information blocks, no document-style long paragraph, no unnecessary floating one-line text boxes.

**🟡 P1 — Serious (strongly recommended)**: 14. ✓ **Text-Background pairs**: For each text with a background shape:
...
15. ✓ No unintended element overlaps (especially check LaTeX elements — their rendered height may be much larger than specified)
