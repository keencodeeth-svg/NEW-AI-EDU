...
const HTML_TEXT_BLOCK_RE = /<(p|h[1-6])\b([^>]*)>([\s\S]*?)<\/\1>/giu;
const HTML_TAG_RE = /<[^>]+>/gu;
const CJK_CHAR_RE = /[\u3400-\u9fff\u3040-\u30ff\uac00-\ud7af]/u;
...
function truncateTextByUnits(value: string, maxUnits: number) {
  if (estimateTextUnits(value) <= maxUnits) return value;
  ...
  return output.trim().replace(/[，。；：、,\-.!?\s]+$/u, '').concat('...');
}

function buildTextDensityBudget(input: {
  width: number;
  height: number;
  textType?: string | null;
}) {
  const { width, height, textType } = input;
  const isHeadline = ['title', 'subtitle', 'header', 'itemTitle'].includes(textType || '');

  if (isHeadline) {
    return {
      maxParagraphs: textType === 'title' ? 1 : 2,
      maxUnitsPerParagraph: width >= 720 ? 30 : width >= 460 ? 26 : 22,
    };
  }

  const maxParagraphs = height < 110 ? 3 : height < 180 ? 4 : 5;
  let maxUnitsPerParagraph = width < 240 ? 16 : width < 360 ? 22 : width < 520 ? 28 : 34;
  if (height < 110) maxUnitsPerParagraph -= 4;
  if (height < 80) maxUnitsPerParagraph -= 3;

  return {
    maxParagraphs,
    maxUnitsPerParagraph: Math.max(12, maxUnitsPerParagraph),
  };
}

function compactHtmlTextContent(
  html: string,
  input: { width: number; height: number; textType?: string | null },
) {
  const normalizedHtml = html.replace(/(?:<br\s*\/?>\s*){3,}/giu, '<br />').trim();
  const blocks = [...normalizedHtml.matchAll(HTML_TEXT_BLOCK_RE)];
  const { maxParagraphs, maxUnitsPerParagraph } = buildTextDensityBudget(input);

  if (blocks.length === 0) {
    const plainText = normalizeTextWhitespace(normalizedHtml.replace(HTML_TAG_RE, ' '));
    if (!plainText) return normalizedHtml;
    return `<p>${escapeHtmlText(truncateTextByUnits(plainText, maxUnitsPerParagraph))}</p>`;
  }

  const nextBlocks = blocks.slice(0, maxParagraphs).map((match, index) => {
    const tagName = match[1];
    const attrs = match[2] || '';
    const ownText = normalizeTextWhitespace(match[3].replace(HTML_TAG_RE, ' '));
    const mergedTail =
      index === maxParagraphs - 1 && blocks.length > maxParagraphs
        ? blocks
            .slice(index)
            .map((item) => normalizeTextWhitespace(item[3].replace(HTML_TAG_RE, ' ')))
            .filter(Boolean)
            .join(' / ')
        : ownText;

    if (!mergedTail) return null;

    return `<${tagName}${attrs}>${escapeHtmlText(
      truncateTextByUnits(mergedTail, maxUnitsPerParagraph),
    )}</${tagName}>`;
  });
...
function normalizeGeneratedSlideTextDensity(
  elements: GeneratedSlideData['elements'],
): GeneratedSlideData['elements'] {
  return elements.map((el) => {
    if (el.type === 'text') {
      const textEl = el as Record<string, unknown>;
      const content = typeof textEl.content === 'string' ? textEl.content : '';
      if (!content.trim()) return el;

      return {
        ...el,
        content: compactHtmlTextContent(content, {
          width: el.width ?? 320,
          height: el.height ?? 120,
          textType: typeof textEl.textType === 'string' ? textEl.textType : undefined,
        }),
      };
    }
    ...
    if (el.type === 'shape') {
      ...
      const shapeText =
        shapeEl.text && typeof shapeEl.text === 'object'
          ? (shapeEl.text as Record<string, unknown>)
          : null;
      const content = shapeText && typeof shapeText.content === 'string' ? shapeText.content : '';
      if (!shapeText || !content.trim()) return el;
      ...
      return {
        ...
        text: {
          ...
          content: compactHtmlTextContent(content, {
            width: el.width ?? 320,
            height: el.height ?? 120,
            textType: typeof shapeText.type === 'string' ? shapeText.type : undefined,
          }),
        },
      };
    }
...
async function generateSlideContent(
  ...
  // Compact over-dense text so the classroom canvas and exported slides stay readable.
  const normalizedElements = normalizeGeneratedSlideTextDensity(fixedElements);
  log.debug(`After text density normalization: ${normalizedElements.length} elements`);

  // Process LaTeX elements: render latex string → HTML via KaTeX
  const latexProcessedElements = processLatexElements(normalizedElements);
