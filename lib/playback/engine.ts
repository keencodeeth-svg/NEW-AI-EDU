    ...
    switch (action.type) {
      case 'speech': {
        ...
        this.audioPlayer
          ...
          .then((audioStarted) => {
            if (!audioStarted) {
              // No pre-generated audio — try browser-native TTS as a universal
              // fallback so classrooms still have audible narration when
              // managed TTS is unavailable or not configured.
