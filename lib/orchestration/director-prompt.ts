  ...
  for (const a of actions) {
    switch (a.actionName) {
      ...
      case 'wb_draw_code': {
        ...
        const fileName = a.params.fileName ? ` "${a.params.fileName}"` : '';
        parts.push(`drew code block${fileName} (${lang || 'code'})`);
        break;
      }
      case 'wb_edit_code': {
        const operation = String(a.params.operation || 'edit');
        parts.push(`edited code (${operation})`);
        break;
      }
      case 'wb_clear':
