...
export async function generateAndStoreTTS(
  ...
  const settings = useSettingsStore.getState();
  const teacherVoice = resolveTeacherVoice(useStageStore.getState().stage?.classroomMeta);
  const effectiveProviderId = teacherVoice.providerId || settings.ttsProviderId;
  const effectiveVoiceId = teacherVoice.voiceId || settings.ttsVoice;
...
async function generateTTSForScene(
  ...
  const settings = useSettingsStore.getState();
  const teacherVoice = resolveTeacherVoice(useStageStore.getState().stage?.classroomMeta);
  const providerId = (teacherVoice.providerId || settings.ttsProviderId) as TTSProviderId;
      ...
      try {
        ...
        for (const outline of pending) {
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            // TTS generation is best-effort. If the managed TTS provider is
            // temporarily unavailable, we still keep the classroom usable and
            // let playback fall back to browser narration.
            if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
              const ttsResult = await generateTTSForScene(scene, signal);
              if (!ttsResult.success) {
                log.warn('Scene TTS degraded to browser narration fallback:', {
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      ...
      try {
        ...
        // Step 3: TTS (best-effort, do not fail the scene when the managed
        // provider is unavailable)
        const settings = useSettingsStore.getState();
        if (settings.ttsEnabled && settings.ttsProviderId !== 'browser-native-tts') {
          const ttsResult = await generateTTSForScene(actionsResult.scene, signal);
          if (!ttsResult.success) {
            log.warn('Retry scene TTS degraded to browser narration fallback:', {
              outlineId,
              failedCount: ttsResult.failedCount,
              error: ttsResult.error,
            });
