...
export async function generateTTSForClassroom(
  ...
  const teacherVoice = resolveTeacherVoice(classroomMeta);
  const providerId = (teacherVoice.providerId || ttsProviderIds[0]) as TTSProviderId;
  ...
  const voice = teacherVoice.voiceId || DEFAULT_TTS_VOICES[providerId] || 'default';
