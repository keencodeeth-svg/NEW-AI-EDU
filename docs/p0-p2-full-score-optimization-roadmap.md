...
- `scripts/visual-check-local.mjs` 已推进到按 `route + viewport + theme` 产出独立 screenshot/report 项，当前首批覆盖 8 个路由：`/`、`/login`、`/register`、`/recover`、`/student`、`/practice`、`/teacher`、`/teacher/classroom-live`。
...
- 下一步仍需继续扩到 `parent`、`school`、`admin`、`ai-classroom`、`student/interactive-classroom`、`school/interactive-classrooms`、`classroom/[id]`，并补足更真实的数据态与异常态，而不是只看壳层稳定性。
