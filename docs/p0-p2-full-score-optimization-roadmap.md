...
| UI 视觉系统与品牌独立性 | 6.9/10 | 统一 logo、色彩、排版、组件、空态、动效和深浅主题；外部用户看不到旧项目或开源模板痕迹 | 旧品牌、内部标签、英文 chip、demo 文案和后台卡片墙仍分散存在 | P0 |
...
| 测试、可访问性与视觉回归 | 7.0/10 | 核心页默认 `critical=0` 且 `serious=0`，全角色视觉门禁覆盖 light/dark + desktop/mobile，79 个页面有覆盖台账 | 视觉脚本当前更像壳层检查；a11y 默认不拦 serious；覆盖页数不足 | P0 |
| 发布、运维与合规 | 7.2/10 | 发布前 readiness、远端 smoke、证书、密钥、回滚、第三方许可与 NOTICE 都有证据 | 证书/部署/开源来源清理需要纳入固定发布门禁，不能靠临时记忆 | P0/P1 |
...
- `scripts/visual-check-local.mjs` 目前覆盖 5 个路由：`/`、`/login`、`/ai-classroom`、`/student`、`/student/interactive-classroom`。
- `tests/browser/a11y.spec.ts` 当前 Axe 产品页覆盖 6 个：`/`、`/login`、`/student`、`/practice`、`/teacher`、`/teacher/classroom-live`。
- `tests/browser/smoke.spec.ts` 触达 24 个 `page.goto` 目标，但主要是业务 smoke，不等价于 UI 门禁。

必须进入 P0 的质量门：

- 核心入口与沉浸页要 100% 覆盖 visual + a11y。
- 默认 Axe 门禁要达到 `critical=0` 且 `serious=0`，不能只在额外环境变量下拦 serious。
...
| P0-5 | 用户可见品牌与旧开源痕迹清理 | `components/RoleSidebarNav.tsx`、`components/brand/*`、`public/logos/*`、`lib/health.ts`、全局文案 | 用户可见层只出现统一品牌；旧 logo、旧 service name、旧项目名、demo 学校文案不出现在 UI；内部 key 迁移有兼容方案；第三方许可和 NOTICE 不被删除 |
...
| P0-7 | a11y 默认拦截 serious | `tests/browser/a11y.spec.ts`、`package.json`、`docs/strict-testing-baseline.md` | 核心页 Axe 默认 `critical=0` 且 `serious=0`；`test:a11y:serious` 不再只是可选增强；CI 或发布前门禁明确包含严口径 a11y |
| P0-8 | 发布前 readiness 与远端 smoke 固定化 | `scripts/launch-readiness-report.mjs`、`scripts/post-deploy-smoke.mjs`、`scripts/deploy-prebuilt-remote.sh`、`docs/staging-production-release-runbook.md` | 发布前固定执行 readiness、构建、核心 smoke、证书检查和回滚预案；HTTPS 证书过期、SSH 恢复、生产服务健康都进入 runbook |
...
PLAYWRIGHT_A11Y_INCLUDE_SERIOUS=true corepack pnpm exec playwright test tests/browser/a11y.spec.ts
...
| 编号 | 优化项 | 涉及路径 | 满分验收 |
| --- | --- | --- | --- |
| P2-1 | 79 个页面路由建立覆盖台账 | `app/**/page.tsx`、`docs/strict-testing-baseline.md`、新增覆盖矩阵文档 | 每页有 owner、用户角色、业务意图、smoke/a11y/visual 归属、empty/loading/error 状态说明 |
...
| P0 | `/ai-classroom` | 课堂目标、对象、课后去向、视觉门禁 |
| P0 | `/student/interactive-classroom` | 体验模式边界、学生学习路径、降级可信度 |
| P0 | `/teacher/classroom-live` | 实时课堂仪表盘、键盘和主题 |
| P0 | `/school/interactive-classrooms` | 学校互动课堂治理中心 |
...
6. 把核心页 a11y 默认门禁提升到 `critical=0` + `serious=0`。
7. 扩容视觉门禁到 P0 核心路由，并固定 light/dark + desktop/mobile。
8. 建立 79 个页面路由覆盖台账，进入 P2 长期满分追踪。

这 8 项完成后，再集中推进 AI 课堂服务端化、全角色状态系统、像素级视觉回归和学习效果证据化。
