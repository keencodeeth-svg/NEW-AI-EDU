...
- `scripts/visual-check-local.mjs` 已推进到按 `route + viewport + theme` 产出独立 screenshot/report 项，当前稳定覆盖 12 个路由：`/`、`/login`、`/register`、`/recover`、`/ai-classroom`、`/student`、`/practice`、`/teacher`、`/teacher/classroom-live`、`/parent`、`/school`、`/admin`。
...
- 下一步仍需继续扩到 `student/interactive-classroom`、`/school/interactive-classrooms`、`/classroom/[id]`，并补足更真实的数据态与异常态，而不是只看壳层稳定性。
