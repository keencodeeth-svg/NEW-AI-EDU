...
- `scripts/visual-check-local.mjs` 已推进到按 `route + viewport + theme` 产出独立 screenshot/report 项，当前稳定覆盖 16 个路由：`/`、`/login`、`/register`、`/recover`、`/ai-classroom`、`/student`、`/practice`、`/student/interactive-classroom`、`/student/exams`、`/teacher`、`/teacher/classroom-live`、`/teacher/lesson-planner`、`/parent`、`/school`、`/school/interactive-classrooms`、`/admin`。
- `tests/browser/a11y.spec.ts` 当前 Axe 产品页覆盖 16 个直接路由：`/`、`/login`、`/register`、`/recover`、`/student`、`/practice`、`/teacher`、`/teacher/classroom-live`、`/ai-classroom`、`/student/interactive-classroom`、`/admin`、`/school`、`/school/interactive-classrooms`、`/parent`、`/library`、`/teacher/lesson-planner`。
- `tests/browser/smoke.spec.ts` 当前归一化触达 27 条页面路由，并已给 `/`、`/practice`、`/parent`、`/school`、`/ai-classroom` 建立独立直达入口 smoke；其余部分仍是业务流程或动态 id 进入，不等价于完整 UI 门禁。
...
- 管理员、教师、学校管理员注册页与账号恢复页不再使用 `@demo.com`、`默认学校`、`航科实验学校` 一类用户可见占位文案。
...
- 2026-06-01 已补入第三批稳定切片：`/student/interactive-classroom` 与 `/school/interactive-classrooms`，视觉矩阵达到 14 条稳定路由；`/classroom/[id]` 仍需动态课堂 fixture 后再纳入。
- 2026-06-01 已补入第四批 P1 壳层切片：`/student/exams` 与 `/teacher/lesson-planner`，视觉矩阵达到 16 条稳定路由，并新增 coverage summary 输出：`16 routes x 2 viewports x 2 themes = 64 checks`。
- 下一步仍需继续补足更真实的数据态与异常态，而不是只看壳层稳定性。

P0-7 当前推进记录：

- `tests/browser/a11y.spec.ts` 已从 public / student / teacher 主线扩展到 16 个直接路由，新增覆盖 `/ai-classroom`、`/student/interactive-classroom`、`/admin`、`/school`、`/school/interactive-classrooms`、`/parent`、`/library`、`/teacher/lesson-planner`。
...
- `/practice`、`/parent`、`/school`、`/ai-classroom` 已新增直达 smoke，确认页面能在正确 session 或公开上下文下落到主工作台、保留主题切换、壳层隔离和 `#main-content`。
...
- `/library` 与 `/teacher/lesson-planner` 已进入 Axe `critical=0 + serious=0` 门禁；`/teacher/lesson-planner` 同时进入 visual light/dark + desktop/mobile 壳层矩阵。
- `/student/exams` 已进入 visual light/dark + desktop/mobile 壳层矩阵；下一步补 Axe、空态和真实考试数据态的视觉断言。
- visual 矩阵已从 14 条稳定路由扩展到 16 条稳定路由，脚本导出 `getVisualCoverageSummary()` 供文档、CI 或后续仪表盘复用。
