...
- smoke：当前归一化触达 27 条路由，其中部分为流程进入或动态 id 进入；`/`、`/practice`、`/parent`、`/school`、`/ai-classroom` 已有独立直达 smoke。
- a11y：当前直接覆盖 `16` 条路由：`/`、`/login`、`/register`、`/recover`、`/student`、`/practice`、`/teacher`、`/teacher/classroom-live`、`/ai-classroom`、`/student/interactive-classroom`、`/admin`、`/school`、`/school/interactive-classrooms`、`/parent`、`/library`、`/teacher/lesson-planner`。
- visual：当前工作区脚本明示覆盖 `16` 条稳定路由：`/`、`/login`、`/register`、`/recover`、`/ai-classroom`、`/student`、`/practice`、`/student/interactive-classroom`、`/student/exams`、`/teacher`、`/teacher/classroom-live`、`/teacher/lesson-planner`、`/parent`、`/school`、`/school/interactive-classrooms`、`/admin`。
- visual 矩阵摘要：按 `route + viewport + theme` 产出，当前为 `16 routes x 2 viewports x 2 themes = 64 checks`；其中 `public 5`、`student 4`、`teacher 3`、`parent 1`、`school 2`、`admin 1`。
...
| `/parent` | 家长 | 查看孩子状态、陪伴动作、鼓励反馈 | 已覆盖 | 已覆盖 | 壳层覆盖 | 未绑定孩子时显示观察码绑定路径；加载中保留家长工作台；提交失败可恢复 | 补未绑定孩子空态与提交失败态 |
| `/school` | 学校管理员 | 查看学校质量治理、课堂应用和排课风险 | 已覆盖 | 已覆盖 | 壳层覆盖 | 无班级/无教师时给导入与邀请路径；加载中保留治理概览；接口失败给只读降级 | 补学校数据空态与接口失败降级 smoke |
| `/admin` | 平台管理员 | 优先处理发布风险、账号恢复、模型链和内容治理 | 已覆盖 | 已覆盖 | 壳层覆盖 | 无工单/无告警时解释健康状态；高风险动作 loading/step-up/error 明确 | 补异常队列空态和 step-up 失败态 |
| `/practice` | 学生 | 进入智能练习并完成学习巩固 | 已覆盖 | 已覆盖 | 壳层覆盖 | 无题目时给生成或选择知识点 CTA；加载题目有 skeleton；提交错误可重试 | 补提交错误态与题目加载态 |
| `/ai-classroom` | 公开 / 学生 / 教师 | 带课堂上下文进入 AI 课堂生成主线 | 已覆盖 | 已覆盖 | 壳层覆盖 | 未带上下文时解释可独立使用；生成中禁用提交；生成失败不伪装成功 | 补生成失败态和课堂卡片键盘流 |
...
| `/library` | 管理员 / 教师 / 学校 | 上传、检索、下载教材与课件 | 已覆盖 | 已覆盖 | 未覆盖 | 无资料时给导入 CTA；上传中禁用；上传、下载、step-up 错误可恢复 | 补列表页 visual 与非管理员只读态 |
...
| `/student/exams` | 学生 | 查看待完成和历史考试 | 已覆盖 | 未覆盖 | 壳层覆盖 | 无考试时给学习建议；加载中保留列表；拉取失败可重试 | 补 Axe、空态与数据态更细 visual 断言 |
...
| `/teacher/lesson-planner` | 教师 | 输入主题并生成备课方案 | 已覆盖 | 已覆盖 | 壳层覆盖 | 空主题校验；生成中禁用；生成失败给重试和人工编辑出口 | 补 AI 失败态与生成后视觉断言 |
...
| `/parent` | `app/parent/page.tsx` | P0 | 家长 | smoke + a11y + visual |
| `/school` | `app/school/page.tsx` | P0 | 学校管理员 | smoke + a11y + visual |
| `/admin` | `app/admin/page.tsx` | P0 | 平台管理员 | smoke + a11y + visual |
| `/practice` | `app/practice/page.tsx` | P0 | 学生 | smoke + a11y + visual |
| `/ai-classroom` | `app/ai-classroom/page.tsx` | P0 | 学生 / 教师 / 公开 | smoke + a11y + visual |
...
| `/library` | `app/library/page.tsx` | P1 | 管理员 / 教师 / 学校 | smoke + a11y |
...
| `/student/exams` | `app/student/exams/page.tsx` | P1 | 学生 | smoke + visual |
...
| `/teacher/lesson-planner` | `app/teacher/lesson-planner/page.tsx` | P1 | 教师 | smoke + a11y + visual |
...
1. 为 `/classroom/[id]` 建立可复现课堂 fixture，并加入 smoke/a11y/visual；这是 P0 核心矩阵里唯一仍没有稳定页面门禁的动态路由。
2. 把 P1 已有 smoke 的列表/详情/高风险操作页补上 Axe 与 visual：`/library/[id]`、`/student/exams/[id]`、`/student/assignments/[id]`、`/teacher/projects`、`/school/schedules`、`/school/classes`、`/admin/ai-models`、`/admin/recovery-requests`。
