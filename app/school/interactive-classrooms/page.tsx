...
export default function SchoolInteractiveClassroomsPage() {
  ...
  if (page.authRequired) {
    return (
      <WorkspaceAuthState
        title="需要学校管理员权限"
        description="请使用学校管理员或平台主管账号登录后查看课堂质量中心。"
      />
    );
  }
  ...
  if (!payload) {
    return (
      <WorkspaceEmptyState
        title="暂无课堂质量数据"
        description="当前学校还没有沉淀课堂使用记录，后续发布、导出或学生自主学习后会自动出现。"
      />
    );
  }
  ...
  const qualitySummaryItems = [
    {
      ...
      helper: `${payload.summary.deliveredClassroomCount} 节课堂已形成使用闭环`,
    },
    {
      label: "整班覆盖",
  ...
  return (
    <WorkspacePage
      title="课堂质量中心"
      subtitle="把课堂发布、课件导出、资源包沉淀与学生自主使用放到同一套学校质量视图里，帮助学校判断真实覆盖、复用效率与学习支持效果。"
    ...
    >
      <div className="grid grid-2" style={{ alignItems: "start" }}>
        <Card title="学校质量摘要" tag="质量">
          <div className="grid" style={{ gap: 14 }}>
            <div className="workflow-summary-grid">
              {qualitySummaryItems.map((item) => (
            ...
            <div className="workflow-card-meta">
              ...
              <span className="pill">全班观看发布与导出归档统一观察</span>
          ...
          <div className="grid" style={{ gap: 14 }}>
            ...
            <div className="card" style={{ padding: 14 }}>
              <div className="section-title">
                {page.activeFilterLabels.length ? "当前已锁定的观察范围" : "当前正在查看学校范围内的全部课堂使用记录"}
              </div>
              <div style={{ marginTop: 8, fontSize: 13, color: "var(--ink-1)", lineHeight: 1.8 }}>
                {topClass
                  ? `当前最热班级为 ${topClass.className}，累计 ${topClass.count} 次课堂使用，最近一次发生在 ${toTimeLabel(topClass.lastDeliveredAt)}。`
                  : "暂时还没有可识别的热点班级，可先让老师完成发布或导出后再观察学校应用速度。"}
              </div>
          ...
          <div className="grid grid-2" style={{ alignItems: "end" }}>
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <input
                ...
                placeholder="搜索班级、课堂、教师、学习者或使用标签"
                aria-label="搜索课堂使用记录"
                style={fieldStyle}
              />
            </label>
            ...
            <label style={{ display: "grid", gap: 6 }}>
              <span className="section-sub">使用动作</span>
              <select
                ...
                {page.teacherAdoptionLeaders.map((item, index) => (
                  ...
                  >
                    <div className="cta-row" style={{ justifyContent: "space-between", gap: 10 }}>
                      ...
                      <span className="pill">使用 {item.deliveryCount}</span>
                    </div>
                    <div style={{ fontSize: 12, color: "var(--ink-1)", marginTop: 4 }}>
                      {item.subject || "未标注学科"}
                      {item.grade ? ` · ${item.grade} 年级` : ""}
              ...
              <div className="grid grid-2" style={{ alignItems: "start" }}>
                {page.studentMomentumLeaders.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="cta-row cta-row-tight" style={{ marginTop: 8, flexWrap: "wrap" }}>
                      <span className="pill">累计使用 {item.deliveryCount}</span>
      ...
      <Card title={`课堂使用记录（${page.filteredRecords.length}）`} tag="明细">
          ...
          <div className="grid" style={{ gap: 12 }}>
            <div className="cta-row" style={{ justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontSize: 13, color: "var(--ink-1)", lineHeight: 1.7 }}>
                默认先展示最近的重点记录，避免首屏列表过长；需要时可以一键展开全部明细。
              </div>
            ...
            {remainingRecordCount > 0 ? (
              <div className="card" style={{ padding: 14 }}>
                ...
                <div style={{ marginTop: 6, fontSize: 13, color: "var(--ink-1)", lineHeight: 1.7 }}>
                  点击“展开全部”可以查看完整记录，或继续使用筛选把视图聚焦到某个年级、学科或课堂模式。
                </div>
              </div>
            ) : null}
          </div>
