...
import { getSchoolClassroomDeliveryDetail } from "@/lib/server/classroom-delivery-ledger";
...
export const GET = createLearningRoute({
  ...
  handler: async ({ user, query }) => {
    ...
    if (user.role === "school_admin") {
      ...
      return { data: await getSchoolClassroomDeliveryDetail(user.schoolId) };
    }
    ...
    return { data: await getSchoolClassroomDeliveryDetail(requestedSchoolId) };
  },
});
