...
import { createQuestion, getKnowledgePoints, getQuestions, updateQuestion } from "@/lib/content";
...
import { crossValidateQuestionAnswer } from "@/lib/question-cross-validate";
import {
...
export const POST = createAdminRoute({
  ...
  handler: async ({ request }) => {
    ...
    for (let i = 0; i < total; i += 1) {
      ...
      const crossValidation = await crossValidateQuestionAnswer(next);
      const validatedQuestion =
        (await updateQuestion(next.id, {
          needsManualReview: crossValidation.needsManualReview,
          reviewReason: crossValidation.reviewReason ?? null
        })) ?? {
          ...next,
          needsManualReview: crossValidation.needsManualReview,
          reviewReason: crossValidation.reviewReason ?? null
      ...
      qualityCandidates.push(validatedQuestion);
      created.push({
        id: validatedQuestion.id,
        actualDifficulty: validatedQuestion.actualDifficulty ?? null,
        needsManualReview: Boolean(validatedQuestion.needsManualReview),
        reviewReason: validatedQuestion.reviewReason ?? null,
