...
export function AgentRevealModal({
  ...
  const cardWidth = isDocked ? 168 : 196;
  const cardHeight = isDocked ? 252 : 290;
      ...
      {open && (
        <motion.div
          className={cn(
            'fixed inset-0 z-[100]',
            isDocked
              ? 'pointer-events-none flex items-end justify-end p-3 sm:p-4'
              : 'flex flex-col items-center justify-center bg-slate-100/88 backdrop-blur-sm dark:bg-black/68 dark:backdrop-blur-md',
          )}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <motion.div
            className={cn(
              ...
              isDocked
                ? `${classroomPanel} max-h-[min(82dvh,780px)] w-full max-w-[min(96vw,560px)] p-4 shadow-[0_28px_90px_rgba(15,23,42,0.18)]`
                : 'flex flex-col items-center justify-center',
            )}
            initial={isDocked ? { opacity: 0, y: 18, scale: 0.98 } : undefined}
            animate={isDocked ? { opacity: 1, y: 0, scale: 1 } : undefined}
            transition={isDocked ? { duration: 0.28, ease: [0.22, 1, 0.36, 1] } : undefined}
          >
            <motion.button
              className={cn(
                'absolute right-4 top-4 z-10 flex h-9 items-center gap-1 rounded-full px-3 text-xs font-medium backdrop-blur-sm transition-colors',
                isDocked
                  ? classroomSoftButton('sky')
                  : 'w-9 justify-center bg-black/5 text-zinc-500 hover:bg-black/10 hover:text-zinc-700 dark:bg-white/10 dark:text-white/70 dark:hover:bg-white/20 dark:hover:text-white',
              )}
            ...
            >
              <X className="size-4" />
              {isDocked ? (allRevealed ? '收起角色卡' : '跳过揭示') : null}
            </motion.button>

            {isDocked ? (
              <div className="pr-16">
                <motion.div
                  className={classroomTonePill('sky', 'inline-flex items-center text-[11px] tracking-[0.12em]')}
                ...
                <motion.h2
                  className="mt-3 text-xl font-bold text-zinc-800 dark:text-white"
                  initial={{ opacity: 0, y: -12 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.12, duration: 0.3 }}
                >
                  <Sparkles className="mr-2 inline-block size-5 text-amber-500 dark:text-yellow-400" />
                  {t('generation.agentRevealTitle')}
                </motion.h2>
                <motion.p
                  className="mt-2 max-w-[46ch] text-sm leading-6 text-slate-600 dark:text-slate-300"
                  initial={{ opacity: 0, y: -10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ delay: 0.16, duration: 0.3 }}
                >
                  角色已经开始就位，课堂会继续在后台生成。你可以边看角色设定，边等待进入正式舞台。
                </motion.p>
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span className={classroomTonePill('sky', 'text-[11px]')}>
                    已揭示 {revealedCount} / {agents.length}
                  </span>
                  <span className={classroomTonePill('slate', 'border-0 ring-1 ring-slate-200 text-[11px] font-medium dark:ring-slate-800')}>
                    {allRevealed ? '课堂继续生成中，可随时收起' : '不想等完整揭示也可以直接跳过'}
                  </span>
                </div>
              </div>
            ) : (
            ...
            <div
              className={cn(
                ...
                isDocked ? 'mt-4 max-h-[min(48dvh,420px)] overflow-y-auto pr-1' : 'px-4 md:gap-5',
              )}
            >
              {agents.map((agent, index) => {
                ...
                return (
                  <motion.div
                    ...
                    style={{ width: cardWidth, height: cardHeight, perspective: 900 }}
                          ...
                          <div className="relative flex size-full flex-col overflow-clip rounded-[14px] bg-white dark:bg-zinc-900">
                            ...
                            <div
                              className={cn(
                                ...
                                isDocked ? 'mt-1' : 'mt-1.5',
                              )}
                            >
                              <h3
                                className={cn(
                                  'max-w-full truncate text-center font-bold tracking-wide',
                                  isDocked ? 'text-[12px]' : 'text-[13px]',
                                )}
                            ...
                            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain px-3.5 pt-1.5 pb-3">
                              <p
                                className={cn(
                                  'text-left leading-[1.65] text-zinc-600 dark:text-zinc-400',
                                  isDocked ? 'text-[10px]' : 'text-[10.5px]',
                                )}
                              >
                                {agent.persona}
                              </p>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            ...
                            {/* Center icon */}
                            <Sparkles className="size-9 text-sky-300/70" />
                            <span className="mt-1.5 text-xl font-bold text-sky-200/60">?</span>
                          </div>
                        </div>
                      </div>
                    </motion.div>
            ...
            <motion.div
              className={cn(
                ...
                isDocked ? 'border-t border-slate-200/70 pt-4 dark:border-slate-800/70' : '',
              )}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              ...
              {allRevealed ? (
                <motion.button
                  ...
                  onClick={onClose}
                >
                  {isDocked ? '继续等待课堂生成' : t('generation.continue')}
                </motion.button>
              ) : isDocked ? (
                <div className="text-center text-[11px] leading-5 text-slate-500 dark:text-slate-400">
                  角色会依次亮相，不影响后续课堂生成。
                </div>
              ) : null}
            </motion.div>
