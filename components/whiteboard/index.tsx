      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute inset-4 pointer-events-auto bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl rounded-3xl shadow-[0_32px_80px_-20px_rgba(15,23,42,0.25)] border-2 border-sky-200/70 dark:border-sky-800/60 flex flex-col overflow-hidden z-[120] ring-4 ring-sky-100/50 dark:ring-sky-900/30"
          >
            {/* Header */}
            <div className="h-14 px-6 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between shrink-0 bg-white/50 dark:bg-gray-800/50">
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  className="relative p-2 text-gray-400 dark:text-gray-500 hover:text-sky-500 dark:hover:text-sky-400 hover:bg-sky-50 dark:hover:bg-sky-900/20 rounded-lg transition-colors"
                  title={t('whiteboard.history')}
                >
                  <History className="w-4 h-4" />
                  {snapshotCount > 0 && (
                    <span className="absolute -top-0.5 -right-0.5 min-w-[16px] h-4 px-1 rounded-full bg-sky-500 text-white text-[10px] font-bold flex items-center justify-center">
                      {snapshotCount}
                    </span>
                  )}
                </motion.button>
            ...
            <div className="flex-1 relative bg-[radial-gradient(#dbeafe_1px,transparent_1px)] dark:bg-[radial-gradient(rgba(56,189,248,0.22)_1px,transparent_1px)] [background-size:24px_24px] overflow-hidden">
              <WhiteboardCanvas />
